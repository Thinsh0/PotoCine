...
export class ForgeGradle3Adapter extends ForgeResolver {
    static logger = LoggerUtil.getLogger('FG3 Adapter');
    static WILDCARD_MCP_VERSION = '${mcpVersion}';
    ...
    static isExecutableJar(version) {
    ...
    async process() {
        ...
        if (!await libRepo.artifactExists(installerPath)) {
            ...
            await libRepo.downloadArtifactByComponents(this.REMOTE_REPOSITORY, LibRepoStructure.FORGE_GROUP, LibRepoStructure.FORGE_ARTIFACT, this.artifactVersion, 'installer', 'jar');
        }
        else {
    ...
    async processVersionManifest(installerOutputDir) {
        ...
        const versionManifestModule = {
            ...
            artifact: this.generateArtifact(versionManifestBuf, await lstat(versionManifestPath), versionRepo.getVersionManifestURL(this.baseUrl, this.minecraftVersion, this.forgeVersion))
        };
    ...
    async processWithoutInstaller(installerPath) {
        ...
        const forgeModule = {
            ...
            artifact: this.generateArtifact(forgeUniversalBuffer, await lstat(universalLocalPath), libRepo.getArtifactUrlByComponents(this.baseUrl, LibRepoStructure.FORGE_GROUP, LibRepoStructure.FORGE_ARTIFACT, this.artifactVersion, 'universal')),
            subModules: []
        };
        ...
        forgeModule.subModules?.push({
            ...
            name: 'Minecraft Forge (version.json)',
            type: Type.VersionManifest,
            artifact: this.generateArtifact(await readFile(versionManifestDest), await lstat(versionManifestDest), this.repoStructure.getVersionRepoStruct().getVersionManifestURL(this.baseUrl, this.minecraftVersion, this.forgeVersion))
        });
